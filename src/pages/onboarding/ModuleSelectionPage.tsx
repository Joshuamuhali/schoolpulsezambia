...
import { Check, Plus, ChevronDown, ChevronUp, ChevronRight, Loader2, Activity } from "lucide-react";
...
const ModuleSelectionPage = () => {
  ...
  const saveSelection = useMutation({
    mutationFn: async (selection: { school_id: string; module_ids: string[] }) => {
      // Insert each selected module with status = pending
      const inserts = selection.module_ids.map((moduleId) => ({
        school_id: selection.school_id,
        module_id: moduleId,
        status: "pending",
        selected_price: modules?.find((m) => m.id === moduleId)?.price_monthly || 0,
      }));
    ...
    onSuccess: () => {
      toast.success("Modules selected! Proceeding to KYC.");
  ...
  const handleProceed = () => {
    ...
    } as { school_id: string; module_ids: string[] });
  };
      ...
      <div className="flex flex-1 items-center justify-center p-8">
        ...
        >
          <div className="text-center lg:text-left">
            ...
            <h1 className="font-display text-3xl font-bold text-primary mb-2">Select Your Modules</h1>
            <p className="mt-1 text-muted-foreground">Choose the features you need for your school</p>
          </div>
